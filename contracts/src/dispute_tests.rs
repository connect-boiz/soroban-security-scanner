...
mod dispute_tests {
    ...
    use crate::{
        ContractError, DisputeStatus, Reputation, SecurityScannerContract,
        SecurityScannerContractClient, Role, UPGRADE_DELAY_KEY, REPUTATION_MAP, Vote,
        DISPUTE_DEADLINE, ADMIN_ROLES,
    };
    ...
    fn set_reputation(env: &Env, contract_id: &Address, researcher: &Address, score: u64) {
        env.as_contract(contract_id, || {
            let mut rep_map: soroban_sdk::Map<Address, Reputation> =
                env.storage().instance().get(&REPUTATION_MAP).unwrap_or(soroban_sdk::Map::new(env));
            let reputation = Reputation {
                researcher: researcher.clone(), score,
                successful_reports: score / 10, total_earnings: 0,
            };
    ...
    fn setup_dispute_env(env: &Env, client: &SecurityScannerContractClient) -> (Address, Address, u64) {
    ...
    fn test_upgrade_mechanism() {
        ...
        client.propose_upgrade(&upgrade_authority, &new_contract, &String::from_str(&env, "2.0.0"), &String::from_str(&env, "Security improvements and new features"));
        ...
        assert_eq!(client.try_execute_upgrade(&upgrade_authority), Err(Ok(ContractError::UpgradeNotReady)));
        advance_timestamp(&env, 604800 + 1);
        client.execute_upgrade(&upgrade_authority);
        let history = client.get_upgrade_history();
        assert_eq!(history.len(), 1);
        assert_eq!(history.get(0).unwrap().from_version, String::from_str(&env, "1.0.0"));
        assert_eq!(history.get(0).unwrap().to_version, String::from_str(&env, "2.0.0"));
    ...
    fn test_emergency_upgrade() {
        ...
        client.initialize(&admin);
        client.emergency_upgrade(&admin, &new_contract, &String::from_str(&env, "1.0.1"), &String::from_str(&env, "Critical security patch for a severe vulnerability that must be fixed immediately"));
    ...
    fn test_upgrade_authority_controls() {
        ...
        let new_contract = test_address(&env, 3);
        client.initialize(&admin);
        assert_eq!(client.try_propose_upgrade(&unauthorized_user, &new_contract, &String::from_str(&env, "2.0.0"), &String::from_str(&env, "Unauthorized upgrade")), Err(Ok(ContractError::Unauthorized)));
        let new_authority = test_address(&env, 4);
        client.set_upgrade_authority(&admin, &new_authority);
        assert_eq!(client.try_propose_upgrade(&admin, &new_contract, &String::from_str(&env, "2.0.0"), &String::from_str(&env, "Admin attempt")), Err(Ok(ContractError::Unauthorized)));
        client.propose_upgrade(&new_authority, &new_contract, &String::from_str(&env, "2.0.0"), &String::from_str(&env, "Authorized upgrade"));
    }
    ...
    fn test_upgrade_delay_configuration() {
        ...
        let default_delay: u64 = env.as_contract(&contract_id, || {
            env.storage().instance().get(&UPGRADE_DELAY_KEY).unwrap_or(604800)
        });
        assert_eq!(default_delay, 604800);
        client.set_upgrade_delay(&admin, &259200);
        let new_delay: u64 = env.as_contract(&contract_id, || {
            env.storage().instance().get(&UPGRADE_DELAY_KEY).unwrap_or(0)
        });
        assert_eq!(new_delay, 259200);
        assert_eq!(client.try_set_upgrade_delay(&admin, &3600), Err(Ok(ContractError::InvalidInput)));
    }
    ...
    fn test_concurrent_upgrade_prevention() {
        ...
        client.initialize(&admin);
        client.set_upgrade_authority(&admin, &upgrade_authority);
        client.propose_upgrade(&upgrade_authority, &new_contract1, &String::from_str(&env, "2.0.0"), &String::from_str(&env, "First upgrade"));
        assert_eq!(client.try_propose_upgrade(&upgrade_authority, &new_contract2, &String::from_str(&env, "3.0.0"), &String::from_str(&env, "Second upgrade")), Err(Ok(ContractError::UpgradeInProgress)));
        client.cancel_upgrade(&upgrade_authority);
        client.propose_upgrade(&upgrade_authority, &new_contract2, &String::from_str(&env, "3.0.0"), &String::from_str(&env, "Second upgrade"));
    }
    ...
    fn test_state_migration_tracking() {
        ...
        client.propose_upgrade(&upgrade_authority, &new_contract, &String::from_str(&env, "2.0.0"), &String::from_str(&env, "Test migration"));
        advance_timestamp(&env, 604800 + 1);
        client.execute_upgrade(&upgrade_authority);
    ...
    fn test_file_dispute_with_stake() {
        ...
        let dispute_id = client.file_dispute(&disputant, &report_id, &String::from_str(&env, "This report is invalid, the vulnerability was already patched"), &evidence);
    ...
    fn test_vote_on_dispute_by_eligible_researcher() {
        ...
        let evidence: soroban_sdk::Vec<BytesN<32>> = soroban_sdk::Vec::new(&env);
        let dispute_id = client.file_dispute(&disputant, &report_id, &String::from_str(&env, "Disputed: vulnerability does not exist"), &evidence);
    ...
    fn test_cannot_vote_with_insufficient_reputation() {
        ...
        let (_admin, _researcher, report_id) = setup_dispute_env(&env, &client);
        let evidence: soroban_sdk::Vec<BytesN<32>> = soroban_sdk::Vec::new(&env);
        let dispute_id = client.file_dispute(&disputant, &report_id, &String::from_str(&env, "Disputed"), &evidence);
        let low_rep_researcher = test_address(&env, 300);
        set_reputation(&env, &contract_id, &low_rep_researcher, 30);
        assert_eq!(client.try_vote_on_dispute(&low_rep_researcher, &dispute_id, &Vote::Accept), Err(Ok(ContractError::InsufficientReputation)));
    }
    ...
    fn test_cannot_vote_twice() {
        ...
        set_reputation(&env, &contract_id, &verifier, 100);
        client.vote_on_dispute(&verifier, &dispute_id, &Vote::Accept);
        assert_eq!(client.try_vote_on_dispute(&verifier, &dispute_id, &Vote::Reject), Err(Ok(ContractError::AlreadyVoted)));
    }
    ...
    fn test_resolve_dispute_after_quorum_dispute_upheld() {
        ...
        let (admin, _researcher, report_id) = setup_dispute_env(&env, &client);
        let evidence: soroban_sdk::Vec<BytesN<32>> = soroban_sdk::Vec::new(&env);
        let dispute_id = client.file_dispute(&disputant, &report_id, &String::from_str(&env, "Disputed: vulnerability is a false positive"), &evidence);
        ...
        assert_eq!(client.try_get_dispute_stake(&dispute_id), Err(Ok(ContractError::DisputeNotFound)));
    }
    ...
    fn test_resolve_dispute_dismissed_stake_forfeited() {
        ...
        assert_eq!(client.resolve_dispute(&dispute_id), DisputeStatus::Dismissed);
    ...
    fn test_resolve_dispute_after_deadline() {
        ...
        let initial_bounty_pool = client.get_bounty_pool();
        let evidence: soroban_sdk::Vec<BytesN<32>> = soroban_sdk::Vec::new(&env);
        let dispute_id = client.file_dispute(&disputant, &report_id, &String::from_str(&env, "Disputed: vulnerability is not valid"), &evidence);
        let verifier = test_address(&env, 200);
        grant_role(&env, &contract_id, &verifier, &Role::Verifier);
        set_reputation(&env, &contract_id, &verifier, 100);
        client.vote_on_dispute(&verifier, &dispute_id, &Vote::Accept);
        assert_eq!(client.try_resolve_dispute(&dispute_id), Err(Ok(ContractError::InvalidDisputeStatus)));
    ...
    fn test_cannot_resolve_already_resolved_dispute() {
        ...
        assert_eq!(client.resolve_dispute(&dispute_id), DisputeStatus::Dismissed);
        assert_eq!(client.try_resolve_dispute(&dispute_id), Err(Ok(ContractError::DisputeAlreadyResolved)));
    }
    ...
    fn test_file_dispute_on_nonexistent_report() {
        ...
        let evidence: soroban_sdk::Vec<BytesN<32>> = soroban_sdk::Vec::new(&env);
        assert_eq!(client.try_file_dispute(&disputant, &99999, &String::from_str(&env, "Dispute on invalid report"), &evidence), Err(Ok(ContractError::NotFound)));
    }
    ...
    fn test_dispute_with_evidence_hashes() {
        ...
        let dispute_id = client.file_dispute(&disputant, &report_id, &String::from_str(&env, "Dispute with evidence hashes"), &evidence);
        let dispute = client.get_dispute(&dispute_id);
        assert_eq!(dispute.evidence_hashes.len(), 2);
        assert_eq!(dispute.evidence_hashes.get(0).unwrap(), BytesN::from_array(&env, &[1; 32]));
        assert_eq!(dispute.evidence_hashes.get(1).unwrap(), BytesN::from_array(&env, &[2; 32]));
    }
    ...
    fn test_file_dispute_on_already_disputed_report() {
        ...
        let (_admin, _researcher, report_id) = setup_dispute_env(&env, &client);
        let evidence: soroban_sdk::Vec<BytesN<32>> = soroban_sdk::Vec::new(&env);
        let dispute_id1 = client.file_dispute(&disputant1, &report_id, &String::from_str(&env, "First dispute on this report"), &evidence);
        let dispute_id2 = client.file_dispute(&disputant2, &report_id, &String::from_str(&env, "Second dispute on same report"), &evidence);
    ...
    fn test_get_dispute_nonexistent() {
        ...
        let _ = setup_dispute_env(&env, &client);
        assert_eq!(client.try_get_dispute(&99999), Err(Ok(ContractError::DisputeNotFound)));
    }
}
