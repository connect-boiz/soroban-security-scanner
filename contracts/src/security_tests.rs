...
mod security_tests {
    ...
    fn test_verify_vulnerability_prevents_double_payout() {
        ...
        SecurityScannerContract::initialize(env.clone(), admin.clone()).unwrap();
        SecurityScannerContract::set_token(env.clone(), admin.clone(), token_address).unwrap();
        ...
        // Fund bounty pool as treasury manager (admin is SuperAdmin)
        SecurityScannerContract::fund_bounty_pool(env.clone(), admin.clone(), 1_000_000).unwrap();
        ...
        // Second verification on same report must be rejected
        let result = SecurityScannerContract::verify_vulnerability(
            env.clone(),
            verifier.clone(),
            report_id,
