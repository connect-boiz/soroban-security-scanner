...
class TimeBasedAttackDetector {
  constructor() {
    ...
    this.vulnerablePatterns = [
      ...
      {
        pattern:
          /\b(?:now|current_timestamp|block\.timestamp)\s*[<>]=?\s*\w+|\b\w+\s*[<>]=?\s*(?:now|current_timestamp|block\.timestamp)/,
      ...
      {
        pattern:
          /lock_period\s*=\s*.*timestamp|timestamp.*lock_period|lock_end\s*=\s*.*timestamp|current_timestamp\s*\+\s*\w+/i,
  ...
  async _scanDirectory(dirPath, vulnerabilities, emergencyStop) {
    ...
    for (const file of files) {
      if (emergencyStop && emergencyStop.isActive()) {
        break;
      }
