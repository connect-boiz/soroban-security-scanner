...
export const EnhancedProgressBar: React.FC<EnhancedProgressBarProps> = ({
  ...
  const renderProgressBar = () => {
    if (indeterminate) {
      return (
        <div
          className={`${sizeClasses[size]} ${bgColorClasses[color]} rounded-full overflow-hidden`}
        >
          <div
...
export const MultiStepProgress: React.FC<MultiStepProgressProps> = ({ steps, className = '' }) => {
  return (
    <div className={`space-y-2 ${className}`}>
        ...
        {steps.map((step, index) => (
          <React.Fragment key={index}>
            ...
            {index < steps.length - 1 && (
              <div className={`flex-1 h-1 ${step.completed ? 'bg-blue-500' : 'bg-gray-300'}`} />
            )}
          </React.Fragment>
        ))}
      </div>
