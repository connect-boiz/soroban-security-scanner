    ...
    return mockTransactions
      .filter(tx => {
        ...
        const matchesSeverity =
          severityFilters.length === 0 || severityFilters.includes(tx.severity);
        ...
        <div className="rounded-xl border border-border p-3 space-y-3">
          ...
          {savedSearches.length > 0 ? (
            ...
            <p className="text-xs text-gray-500">
              No saved searches yet. Save commonly used filter combinations.
            </p>
          )}
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-border">
              <th
                className="p-4 font-semibold text-sm cursor-pointer hover:bg-gray-100"
            ...
            {paginatedData.length > 0 ? (
              paginatedData.map(tx => (
                ...
                >
                  ...
                  <td className="p-4">
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${getSeverityClass(tx.severity)}`}
                    >
                  ...
                  <td className="p-4">
                    <span
                      className={`text-xs font-semibold ${tx.status === 'completed' ? 'text-green-600' : tx.status === 'failed' ? 'text-red-600' : 'text-blue-600'}`}
                    >
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="mt-2 p-4 bg-gray-50 rounded-lg border border-border">
                  <p className="text-sm">
                    Found <span className="font-bold">{selectedTx.issuesFound}</span> security
                    vulnerabilities with a maximum severity of{' '}
                    <span
                      className={`font-bold ${selectedTx.severity === 'critical' ? 'text-red-600' : 'text-orange-600'}`}
                    >
                      {selectedTx.severity.toUpperCase()}
                    </span>
                    .
                  </p>
                </div>
              </div>
              <button className="btn btn-primary w-full mt-4" onClick={() => setSelectedTx(null)}>
                Close
              </button>
            </div>
          </div>
