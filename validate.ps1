...
if ($kubectlAvailable) {
    ...
    Get-ChildItem "k8s/*.yaml" | ForEach-Object {
        try {
            $result = kubectl apply --dry-run=client -f $_.FullName 2>$null
            if ($LASTEXITCODE -eq 0) {
                Write-Host "  ✅ $($_.Name) (valid syntax)" -ForegroundColor Green
            } else {
                Write-Host "  ❌ $($_.Name) (invalid syntax)" -ForegroundColor Red
                Write-Host "    Run: kubectl apply --dry-run=client -f $($_.FullName)" -ForegroundColor Yellow
            }
        } catch {
            Write-Host "  ⚠️  $($_.Name) (validation skipped)" -ForegroundColor Yellow
        }
    }
} else {
