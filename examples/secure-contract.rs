...
impl SecureTimeLock {
    ...
    pub fn create_lock(
        ...
        let lock_end_block = current_block
            .checked_add(lock_duration_blocks)
        ...
        env.storage()
            .instance()
            .set(&Symbol::new(&env, "lock_end_block"), &lock_end_block);
        env.storage().instance().set(
            &Symbol::new(&env, "lock_end_timestamp"),
            &lock_end_timestamp,
        );
        env.storage()
            .instance()
    ...
    pub fn release_funds(env: Env) -> Result<(), &'static str> {
        ...
        let lock_end_block: u32 = env
            .storage()
            .instance()
            .get(&Symbol::new(&env, "lock_end_block"))
            .ok_or("Lock not found")?;

        let lock_end_timestamp: u64 = env
            .storage()
            .instance()
        ...
        if current_block >= lock_end_block {
            ...
            if timestamp_drift <= 300 {
                ...
                env.storage()
                    .instance()
                    .set(&Symbol::new(&env, "nonce"), &nonce);

                // Clear lock data
                env.storage()
                    .instance()
                    .remove(&Symbol::new(&env, "lock_end_block"));
                env.storage()
                    .instance()
                    .remove(&Symbol::new(&env, "lock_end_timestamp"));
    ...
    pub fn extend_lock(
        ...
        let mut lock_end_block: u32 = env
            .storage()
            .instance()
            .get(&Symbol::new(&env, "lock_end_block"))
            .ok_or("Lock not found")?;
        ...
        env.storage()
            .instance()
            .set(&Symbol::new(&env, "lock_end_timestamp"), &new_timestamp);
